#!/opt/bin/ash
. "${BRORAY_LIGHT_ROOT_PREFIX:-}/opt/broray-light/lib/web-publication-environment.sh" || exit 1
...
BASELINE_CONFIG_SHA256='b6966860dbe7f074aebe195f956d4fd572666c943b716803fd9422a40ec0420b'
...
ndns_identity()
{
    target="$BRL_WEB_WORK/web-publish-ndns.$$.out"
...
{
    host="$1"
    config="$BRL_WEB_WORK/web-publish-verify.$$.conf"
...
{
    config="$BRL_WEB_WORK/web-publish-absent.$$.conf"
    running_config "$config" || return 1
...
owner_write()
{
    host="$1"
    ndns_base="$2"
    config="$BRL_WEB_WORK/web-publish-owner-profile.$$.conf"
    running_config "$config" || return 1
...
ensure_publication()
{
    ...
    mkdir -p "$BRL_WEB_WORK" "$APP_ROOT/config" || return 1
    ...
    before="$BRL_WEB_WORK/web-publish-before.$$.conf"
    config_backup="$BRL_WEB_WORK/web-publish-lighttpd-before.$$.conf"
    owner_backup="$BRL_WEB_WORK/web-publish-owner-before.$$.json"
    ...
    final="$BRL_WEB_WORK/web-publish-final.$$.conf"
...
status_publication()
{
    config="$BRL_WEB_WORK/web-publish-status.$$.conf"
    mkdir -p "$BRL_WEB_WORK" || return 1
    running_config "$config" || return 1
...
delete_publication()
{
    config="$BRL_WEB_WORK/web-publish-delete.$$.conf"
    mkdir -p "$BRL_WEB_WORK" || return 1
    running_config "$config" || return 1
