        ...
        if [ "$sync_active_name_changed" = "true" ] &&
           [ "$sync_active_config_changed" != "true" ]; then
            if ! broray_interface_sync_description >/dev/null 2>&1; then
                if ! broray_server_subscription_restore_backup \
                    "$sync_subscription_id" "$sync_backup" "$sync_active_before"; then
                    # Preserve the backup for explicit recovery; never claim rollback.
                    broray_server_subscription_release_lock
                    broray_server_subscription_error \
                        "SUBSCRIPTION_RECOVERY_REQUIRED" \
                        "Не удалось восстановить подписку после ошибки синхронизации имени. Резервная копия сохранена: $sync_backup"
                    return 1
                fi
                rm -rf "$sync_work"
                broray_server_subscription_release_lock
