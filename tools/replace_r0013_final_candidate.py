...
BUILD = REPO / 'dist/R0013/p97-release-build/A'
OLD = REPO / 'dist/R0013/p83-build-A'
OUT = REPO / 'dist/R0013/private-target/p100'
REPORT = REPO / 'checkpoints/R0013/TARGET-INSTALL-P100.json'
assert not OUT.exists() and not REPORT.exists()
OUT.mkdir()
report = {'schemaVersion':1,'revision':'p100-final-package-v1','status':'IN_PROGRESS','sourceCommit':'c2c5afd635e6e0e39c9f9f34170e801ce89302d0','candidateReady':False,'steps':[]}
...
test "$(sha256sum /opt/broray-light/current/APP-SHA256SUMS | awk '{print $1}')" = 57832490c248a75d0286f142c8553e4b5e7a377336b10f648ee2e2dffb206716
(cd /opt/broray-light/current && sha256sum -c APP-SHA256SUMS >/dev/null)
test ! -e /opt/broray
test ! -e /opt/etc/init.d/S24broray
mktemp -d /tmp/brl-r13-final-p100.XXXXXX
''').strip()
    assert re.fullmatch(r'/tmp/brl-r13-final-p100\.[A-Za-z0-9]{6}',work)
    target.work=work
...
test "$(sha256sum /opt/broray-light/current/APP-SHA256SUMS | awk '{print $1}')" = 26885a8bfe38beec240c26cb9bd61f334070239ed8e201b35ef5bb6f349c81d8
(cd /opt/broray-light/current && sha256sum -c APP-SHA256SUMS >/dev/null)
...
''')
    ...
    report['status']='PASS_EXACT_P97_INSTALLED_EXISTING_DURABLE_FILES_PRESERVED'
