...
. "$RAM_LIBRARY" || exit 1
REQUEST_LOCK="$ROOT_PREFIX/tmp/broray-light-updater/request.lock"
GLOBAL_LOCK="$ROOT_PREFIX/tmp/broray-light/run/locks/global-operation.lock"
WORK_ROOT=''
WORK_ID=''
...
STATE_FILE="$ROOT_PREFIX/tmp/broray-light-updater/state.json"
...
atomic_text()
{
    ...
    (umask 077; set -C; cat > "$temporary") || return 1
...
{
    case "$1" in
        "$REQUEST_LOCK") brl_lock_acquire request ;;
        "$GLOBAL_LOCK") brl_lock_acquire updater-global ;;
        *) return 3 ;;
    esac
}
...
{
    case "$1" in
        "$REQUEST_LOCK") brl_lock_release request ;;
        "$GLOBAL_LOCK") brl_lock_release updater-global ;;
        *) return 3 ;;
    esac
}
...
update_locks_cleanup()
{
    ...
    if [ "$UPDATE_GLOBAL_ACQUIRED" = true ]; then
        lock_release "$GLOBAL_LOCK" || rc=1
        [ "$rc" -ne 0 ] || UPDATE_GLOBAL_ACQUIRED=false
    fi
    if [ "$UPDATE_REQUEST_ACQUIRED" = true ]; then
        lock_release "$REQUEST_LOCK" || rc=1
        [ "$rc" -ne 0 ] || UPDATE_REQUEST_ACQUIRED=false
    fi
    return "$rc"
}
...
runtime_cleanup()
{
    ...
    if [ -n "$WORK_ROOT" ]; then
        if brl_ram_prepare && brl_ram_dir_valid "$WORK_ROOT" &&
            [ "$(stat -c '%d:%i' "$WORK_ROOT")" = "$WORK_ID" ]; then
            case "$WORK_ROOT" in
                "$BRL_UPDATER_RAM/work/request."??????) rm -rf -- "$WORK_ROOT" || rc=1 ;;
                *) rc=1 ;;
            esac
        else
            echo 'ERROR: Work cleanup refused; preserve RAM evidence.' >&2
            rc=1
        fi
    fi
    update_locks_cleanup || rc=1
    exit "$rc"
}
...
{
    local path
    for path in "$ROOT_PREFIX/opt/broray" "$ROOT_PREFIX/opt/etc/init.d/S24broray" \
                "$ROOT_PREFIX/opt/var/lock/broray/global-operation.lock"; do
...
{
    local path
    coownership_clear || return 1
    brl_ram_prepare || return 1
    # Never adopt symlinked durable product/receipt containers.
    for path in "$APP_ROOT" "$RELEASES_ROOT" "$ROOT_PREFIX/opt/var" "$ROOT_PREFIX/opt/var/lib"; do
        [ -d "$path" ] && [ ! -L "$path" ] &&
    ...
    [ -d "$STATE_ROOT" ] && [ ! -L "$STATE_ROOT" ] &&
        [ "$(stat -c '%u' "$STATE_ROOT")" = 0 ] || return 1
    if [ -z "$WORK_ROOT" ]; then
        WORK_ROOT="$(umask 077; mktemp -d "$BRL_UPDATER_RAM/work/request.XXXXXX")" || return 1
        brl_ram_dir_valid "$WORK_ROOT" || return 1
        WORK_ID="$(stat -c '%d:%i' "$WORK_ROOT")" || return 1
    fi
}
...
fetch_file()
{
    ...
    case "$output" in "$WORK_ROOT/"*) ;; *) return 1 ;; esac
    [ ! -L "$output" ] && [ ! -d "$output" ] || return 1
    curl -q --proto '=https,file' --proto-redir '=https' --tlsv1.2 \
        --connect-timeout 15 --max-time 600 --retry 2 --max-filesize "${3:-1048576}" -fsSL \
...
archive_safe()
{
    ...
    [ -s "$listing" ] || return 1
    [ -z "$(sed 's:/$::' "$listing" | sort | uniq -d)" ] || return 1
...
command_check()
{
    ...
    if [ "$UPDATE_REQUEST_ACQUIRED" != true ]; then
        lock_acquire "$REQUEST_LOCK" check || return 1
...
command_update()
{
    ...
    case "$relation" in
        equal) state_write success no-op 'Installed release equals signed release; no mutation.'; command_check || return 1; return 0 ;;
    ...
    fetch_file "$bundle_url" "$bundle" "$bundle_size" || { state_write error download 'Application slot download failed.' DOWNLOAD_FAILED || true; return 1; }
    ...
    staging="$WORK_ROOT/slot"; final="$RELEASES_ROOT/$available"
    [ ! -e "$final" ] && [ ! -L "$final" ] || { state_write error stage 'Target release slot already exists.' TARGET_EXISTS || true; return 1; }
    ...
    # Only validated installed payload enters /opt; the adjacent slot enables
    # same-filesystem publication. It is a durable transaction target, not scratch.
    local adjacent
    adjacent="$RELEASES_ROOT/.verified-$available-$"
    [ ! -e "$adjacent" ] && [ ! -L "$adjacent" ] || return 1
    (umask 077; mkdir "$adjacent") || return 1
    cp -pR "$staging/." "$adjacent/" || return 1
    slot_valid "$adjacent" "$available" "$candidate" "$files" "$bytes" || return 1
    mv -T "$adjacent" "$final" || return 1
    previous="$installed"
...
command_recover()
{
    ...
    ensure_layout || return 1
    lock_acquire "$REQUEST_LOCK" recover || return 1
    UPDATE_REQUEST_ACQUIRED=true
    lock_acquire "$GLOBAL_LOCK" recover || return 1
    UPDATE_GLOBAL_ACQUIRED=true
...
case "${1:-}" in
    ...
    status) brl_ram_prepare || exit 1; [ ! -L "$STATE_FILE" ] || exit 1; [ -r "$STATE_FILE" ] && cat "$STATE_FILE" || printf '%s\n' '{"state":"idle"}' ;;
