...
Постоянный объём — примерно **35 МиБ вместе с Xray**. В сборке 2.0.0 собственный app-slot Light содержит 146 файлов, 858083 байт (около 0,82 МиБ); отдельно находятся обновитель, настройки и Xray 26.9.9 размером 35061884 байта (около 33,44 МиБ). Объём пользовательского каталога зависит от числа серверов и подписок; фактический расход файловой системы может отличаться.
...
[ "$(sha256sum "$D/install.sh" | awk 'NR==1{print $1}')" = \
  'd58e07633d79780e4102505f2db16c5e93328cd7956d6654c02a77940527ba82' ]
