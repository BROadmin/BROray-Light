...
/opt/bin/tar --version | grep -q "GNU tar"
printf capability | /opt/bin/gzip -c | /opt/bin/gzip -t
work="$(mktemp -d /tmp/brl-r13-backup-p74.XXXXXX)"
[ "$(find -P "$work" -maxdepth 0 -type d -uid 0 -printf '%m')" = 700 ]
printf 'R0013 P74 private target backup\n' >"$work/owner"
printf '%s\n' "$work"
set --
...
/opt/bin/tar --exclude='opt/broray/run' --exclude='opt/broray/logs' --exclude='opt/broray/tmp' --exclude='opt/broray/update' -czf "$work/full-broray.tar.gz" -C / "$@"
/opt/bin/gzip -t "$work/full-broray.tar.gz"
